...
bool esAvlAux(int alt,bool AVL,TAbb a){
	if (a == NULL) 
	  AVL = true;	  
	  else if(a!=NULL && alt!=0 && esAvlAux(alt-1,AVL,izquierdo(a)) && esAvlAux(alt-1,AVL,derecho(a)) && abs(altura(izquierdo(a)) - altura(derecho(a)))<2){
		  AVL = true;
	   }
	   else AVL = false;
  return AVL;	   
...
bool esAvl(TAbb abb) {
	bool avl_aux = true;
	return esAvlAux(altura(abb),avl_aux,abb);	
}	

TAbb avlMin_aux(nat &ini,nat h, TAbb avl){
  if(h>0){
	if(h==1){
		TInfo info= crearInfo(ini,0);
		avl = consAbb(info,NULL,NULL);
		ini++;
	}
	else{
		TAbb izq = avlMin_aux(ini,h-1,avl);
		TInfo infoR = crearInfo(ini,0);
		ini++;
	    TAbb der = avlMin_aux(ini,h-2,avl);
	    avl = consAbb(infoR,izq,der);
	}
   }	
	return avl;	
}	
...
TCadena filtradaOrdenada(TCadena cad, TIterador it) {
	...
	TMapping filtradaOrd = crearMap(N);
	TCadena result = crearCadena();
	if(N==0 || !estaDefinidaActual(it)) result = NULL;
	else{
	    for(nat i = 0; i<N;i++){
	      filtradaOrd= asociarEnMap(natInfo(primeroEnCadena(cad)),realInfo(primeroEnCadena(cad)),filtradaOrd);
	      cad = cadenaSiguiente(cad);
	     }
	     while(estaDefinidaActual(it)){
	        double real = 0;
	        nat actualIt =actualEnIterador(it);
	      if(esClaveEnMap(actualIt,filtradaOrd)){
	        while(esClaveEnMap(actualIt,filtradaOrd)){
			    real = valorEnMap(actualIt,filtradaOrd)+real;
		        filtradaOrd = desasociarEnMap(actualIt,filtradaOrd);
		      }  
	     result = insertarAlFinal(actualIt,real,result);
	      }
	     avanzarIterador(it); 
		}
	 }
  liberarMap(filtradaOrd);
  return result;
}
